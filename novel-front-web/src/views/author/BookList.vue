                ...
                >
                  <td style="position: relative" class="goread">
                    <img
                      ...
                      :alt="item.bookName"
                    /><br />
                    <a href="javascript:void(0)" class="book-name-link" @click="viewChapters(item.id)">{{ item.bookName }}</a>
                  </td>
                  ...
                  <td class="goread" id="opt1431636515973345292">
                    <div class="action-links">
                      <router-link class="action-link publish-link" :to="{'name':'authorChapterAdd','query':{'id':item.id}}">章节发布</router-link>
                      <a href="javascript:void(0)" class="action-link delete-link" @click="confirmDelete(item)">删除</a>
                    </div>
                  </td>
                </tr>
              </tbody>
...
export default {
  ...
  setup() {
    ...
    const getBookImageUrl = (imageUrl) => {
      return getImageUrl(imageUrl, '/assets/images/pic_upload.png');
    };

    // 确认删除小说
    const confirmDelete = (book) => {
      ElMessageBox.confirm(
        `确定要删除小说《${book.bookName}》吗？删除后无法恢复！`,
        '删除确认',
        {
    ...
    const viewChapters = (bookId) => {
      router.push({ name: 'authorChapterList', query: { id: bookId } });
    };
