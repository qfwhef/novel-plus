...
import com.wcoal.novelplus.core.common.utils.IpUtils;
...
public class FlowLimitInterceptor implements HandlerInterceptor {
    ...
    static {
        ...
        rule1.setResource(NOVEL_RESOURCE);//资源名
        rule1.setGrade(RuleConstant.FLOW_GRADE_QPS);//QPS模式
        rule1.setCount(2000);//QPS阈值
        rule1.setControlBehavior(RuleConstant.CONTROL_BEHAVIOR_RATE_LIMITER);//流控行为，默认是快速失败
        rules.add(rule1);
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            entry = SphU.entry(NOVEL_RESOURCE, EntryType.IN, 1, ip);// 进入资源
